...
1. Extensions that do not apply in the context of the public Internet (such as an `extKeyUsage` value for a service that is only valid in the context of a privately managed network), unless:<br>
   ...
   iii. the extension is defined within an open standards specification and intended for use by other organizations. A Certificate that includes such an extension MUST confirm to the specifications of the open standard and of these Requirements.
